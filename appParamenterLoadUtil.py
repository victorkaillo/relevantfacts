...
class AppParameterLoadUtil(metaclass=Singleton):
    ...
    def _show_help(cls):
        help_message = """
        Create config_asterisk_manager.cfg file in your $HOME environment 
        in the following format for the AsteriskManager application to work:
        """
        format_to_config_asterisk_manager = """
        [logging_config]
        level=DEBUG
        [asterisk_manager]
        guru_host = https://url.sample.com
        ip = 123.123.123.123
        username = usernamesample
        password = passwordsample
        """
        logging.info(
                    f"{help_message} {format_to_config_asterisk_manager}"
        )
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    CONFIG_PARAMS = appParameterLoadUtil.get_dictinary(
        file_name='config_asterisk_manager.cfg', section_name='asterisk_manager')
